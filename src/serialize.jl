...
function serialize(s::SQLSyntax)
    @dissect(s, WITH_CONTEXT(tail = (local s′), dialect = (local dialect), table = (local table))) || throw(IllFormedError())
    ctx = SerializeContext(dialect, s′)
    serialize!(ctx)
    raw = String(take!(ctx.io))
    SQLString(raw, table = table, vars = ctx.vars)
end
